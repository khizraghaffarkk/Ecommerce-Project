...
<html lang="en">
    ...
    <header class="jumbotron back-image" style="background-image: url(Images/UniImages/sh6.jpg);">
    ...
    <h2 class="text-center mt-3">SHOES PRODUCTS</h2>
    </div>
    ...
    $db=new dbconn();
    $save_res=$db->loaddata2();
    foreach($save_res as $row){
      //print_r($save_res);
      ?>
         <!-- Start 1st Card -->
         <div class="col-lg-3 col-sm-6">
            <form action="ShoesAddToCart.php" method="post">
             <div class="card mb-3">
               <div class="card-body text-center pt-0 pl-0 pr-0 pb-0">
                  <img class="card-img-top square" src="<?php echo $row['Shoes_image']  ?>" alt="Card image cap">
                  <p class="blockquote-footer mt-3"><?php echo $row['Shoes_des'] ?></p>
                  <p class="m">Rs.<?php echo $row['Shoes_price'] ?></p>
                  <input type="hidden" name="shoes_id" value="<?php echo $row['Shoes_ID']  ?>" >             
                  <input type="hidden" name="shoes_des" value="<?php echo $row['Shoes_des']  ?>" >             
                  <input type="hidden" name="shoes_price" value=" <?php echo $row['Shoes_price']  ?>" >             
                 
                  <input class="text-center form-control" type="number" name="shoesquantity" min="1" max="5" placeholder="Quantity: " >
               </div>
