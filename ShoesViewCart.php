...
if(isset($_GET['id'])){
    ...
    // $db4=new dbconn();
    // $check=$db4->deleteOrder($id);
    // echo "Delete ORder";
    unset($_SESSION['shoesproductcart'][$id]);
    unset($_SESSION['shoesproductquantity'][$id]);
}
if(isset($_SESSION['shoesproductcart']) && !empty($_SESSION['shoesproductcart'])){
    $grandtotal=array();?>
...
<html lang="en">
    ...
    <div class="jumbotron dressclass pt-1 pb-1">
      <div class="container">
         <div class="row mt-3">
 <?php
foreach($_SESSION['shoesproductcart'] as $key => $value){

    $qty=$_SESSION['shoesproductquantity'][$key];
    require_once("dbconn.php");
    $db1=new dbconn();
    $cart_detail=$db1->ShoesforView_cartData($value);
    $subtotal=$cart_detail['Shoes_price'] * $qty;
 ?>
         <!-- Start 1st Card -->
            ...
            <form action="AddToCart.php" method="post">
             <div class="card">
               <div class="card-body text-center pt-0 pl-0 pr-0 pb-0">
                  <img class="card-img-top square" src="<?php echo $cart_detail['Shoes_image']  ?>" alt="Card image cap">
                  <p class="blockquote-footer mt-3"><?php echo $cart_detail['Shoes_des'] ?></p>
                  <p class="m">Rs.<?php echo $cart_detail['Shoes_price'] ?></p>
               ...
               <?php echo "<a href='Shoes.php' style='width: 103px;' class='btn btn-dark'>Back</a>"; ?>
               </div>
             </div>
             </form>
           </div>
    ...
    if(!isset($_GET['id'])){
      require_once("dbconn.php");
      $db3=new dbconn();
      $check=$db3->ShoesinsertOrder($value,$qty,$subtotal);
      }
      // if(isset($check)){ 
      //   echo "Added the record";
      // }
    //$order=array();
    //$order[]=$cart_detail['Product_color'];
}
