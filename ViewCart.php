...
if(isset($_GET['id'])){
    ...
    unset($_SESSION['productquantity'][$id]);
}
...
foreach($_SESSION['productcart'] as $key => $value){
    ...
    $qty=$_SESSION['productquantity'][$key];
    require_once("dbconn.php");
    $db1=new dbconn();
    $cart_detail=$db1->forView_cartData($value);
    $subtotal=$cart_detail['Product_price'] * $qty;
 ?>
         <!-- Start 1st Card -->
            ...
            <form action="AddToCart.php" method="post">
             <div class="card">
               <div class="card-body text-center pt-0 pl-0 pr-0 pb-0">
                  <img class="card-img-top square" src="<?php echo $cart_detail['Product_image']  ?>" alt="Card image cap">
                  <p class="blockquote-footer mt-3"><?php echo $cart_detail['Product_color'] ?></p>
                  <p class="m">Rs.<?php echo $cart_detail['Product_price'] ?></p>
               ...
               <?php echo "<a href='UniProject.php' style='width: 103px;' class='btn btn-dark'>Back</a>"; ?>
               </div>
             </div>
             </form>
           </div>
    ...
    if(!isset($_GET['id'])){
      require_once("dbconn.php");
      $db3=new dbconn();
      $check=$db3->insertOrder($value,$qty,$subtotal);
      }
      // if(isset($check)){ 
